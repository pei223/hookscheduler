...
type Task struct {
	TaskID   uuid.UUID `boil:"task_id" json:"task_id" toml:"task_id" yaml:"task_id"`
...
var TaskWhere = struct {
	TaskID   whereHelperuuid_UUID
	TaskName whereHelperstring
}{
	TaskID:   whereHelperuuid_UUID{field: "\"tasks\".\"task_id\""},
	TaskName: whereHelperstring{field: "\"tasks\".\"task_name\""},
}
...
var (
	taskAllColumns            = []string{"task_id", "task_name"}
	taskColumnsWithoutDefault = []string{"task_id", "task_name"}
	taskColumnsWithDefault    = []string{}
	taskPrimaryKeyColumns     = []string{"task_id"}
...
func FindTask(ctx context.Context, exec boil.ContextExecutor, taskID uuid.UUID, selectCols ...string) (*Task, error) {
	taskObj := &Task{}
	...
	query := fmt.Sprintf(
		"select %s from \"tasks\" where \"task_id\"=$1", sel,
	)
...
func (o TaskSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"tasks\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, taskPrimaryKeyColumns, len(o))
...
func (o *Task) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindTask(ctx, exec, o.TaskID)
	if err != nil {
		return err
	}
...
func TaskExists(ctx context.Context, exec boil.ContextExecutor, taskID uuid.UUID) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"tasks\" where \"task_id\"=$1 limit 1)"
...
func (o *Task) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return TaskExists(ctx, exec, o.TaskID)
}
